...
/**
 * Files the request as a Lead in Aleesa, tagged `property-checklist` so it is
 * distinguishable from a plain newsletter signup. Throwing is what drives the
 * error state below.
 *
 * TODO(launch): the checklist itself still has to be *sent*. Either add an
 * Aleesa automation on the `property-decision-checklist` form, or attach the
 * email provider — see `src/app/api/newsletter/route.ts`.
 */
async function requestChecklist(email: string): Promise<void> {
  const response = await fetch("/api/newsletter", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email, source: "property-checklist" }),
  });
